# Filename: app.py (Single File Version - Sidebar Nav, Audio Crop, Improved Prompts)
...
# ------------------- STYLING -------------------
# (Keep get_base64_of_bin_file and set_styles functions as they were - white text version)
...
# ------------------- DATABASE FUNCTIONS -------------------
# (Keep init_db, save_user, authenticate, save_audio_data functions as they were)
def init_db():
    try:
...
# ------------------- EMAIL FUNCTIONS -------------------
# (Keep send_email, send_fraud_report, send_feedback_email functions as they were)
...
def send_fraud_report(user_email, classification, fraud_reason, file_name):
    ...
    if send_email(subject, body): st.success(f"{classification} report sent!")

def send_feedback_email(user_email, feedback_text):
    subject = f"📝 User Feedback: {user_email}"
    body = f"Feedback submitted.\n\nUser: {user_email}\n\nFeedback:\n-----------------\n{feedback_text}\n-----------------\n\nTimestamp: {time.strftime('%Y-%m-%d %H:%M:%S')}"
    if send_email(subject, body): st.success("Feedback submitted!")

# ------------------- AUDIO PROCESSING FUNCTION -------------------
# (Keep process_audio function as it was)
def process_audio(audio_bytes, original_filename):
    ...
    try:
        ...
        if len(audio_segment) > MAX_AUDIO_DURATION_MS:
            ...
            cropped = True
            st.info(f"Audio cropped to the first {MAX_AUDIO_DURATION_MS/1000} seconds for processing.")
        else:
...
FRAUD_ANALYSIS_PROMPT = (
    ...
    "Analyze the audio recording meticulously and classify it into ONE category: 'Fraud', 'Spam', 'Normal', or 'Unclear/Empty'."
    ...
    "Line 2 onwards: A concise (1-3 sentences) justification explaining the key reasons. **For 'Normal' calls that were unsolicited or slightly vague (like the proactive notification type), explicitly state *why* it wasn't classified as Spam or Fraud (e.g., 'Caller identified as Flipkart regarding an order, no sensitive info/payment demanded, classifying as Normal despite vagueness.')**"
...
# (Keep get_gemini_response function as it was - includes the fix for 'path')
def get_gemini_response(prompt, audio_bytes, mime_type):
    ...
    try:
        audio_file_object = io.BytesIO(audio_bytes)
        uploaded_file = genai.upload_file(path=audio_file_object, mime_type=mime_type) # Use path=
        print(f"Audio uploaded: {uploaded_file.name}. Waiting...")
        while uploaded_file.state.name == "PROCESSING": time.sleep(2); uploaded_file = genai.get_file(uploaded_file.name)
        if uploaded_file.state.name == "FAILED": raise ValueError(f"Gemini file processing failed: {uploaded_file.name}")
        if uploaded_file.state.name != "ACTIVE": raise ValueError(f"Uploaded file not active: {uploaded_file.state.name}")
        print("Generating content..."); response = gemini_model.generate_content([prompt, uploaded_file])
        result_text = response.text.strip(); print(f"Gemini raw response captured.")
        try: print(f"Deleting Gemini file: {uploaded_file.name}"); genai.delete_file(uploaded_file.name); print(f"Deleted.")
        except Exception as del_e: print(f"Warning: Could not delete Gemini file {uploaded_file.name}: {del_e}")
        return result_text
    except Exception as e:
        error_msg = f"Gemini API Error: {e}"; print(error_msg)
        if uploaded_file and hasattr(uploaded_file, 'name'):
            try: print(f"Deleting Gemini file after error: {uploaded_file.name}"); genai.delete_file(uploaded_file.name); print(f"Deleted after error.")
            except Exception as del_e: print(f"Could not delete Gemini file {uploaded_file.name} after error: {del_e}")
        return f"Error: {error_msg}"

# --- RESPONSE PARSING ---
# (Keep parse_fraud_analysis_response function as it was)
def parse_fraud_analysis_response(response_text):
    ...
    valid_classifications = ["Fraud", "Spam", "Normal", "Unclear/Empty"]
...
# (Keep show_welcome_page, fraud_analysis_page, transcribe_page, feedback_page functions as they were)
...
def fraud_analysis_page():
    ...
    if uploaded_file:
        ...
        if st.button("Analyze Call", key=analyze_button_key):
            ...
            if processed_audio_bytes:
                ...
                with st.spinner("Analyzing with AI..."):
                    ...
                    else: st.error(f"Analysis Failed: {reason}")
                    st.rerun()
        if result_key in st.session_state:
            ...
            else: st.info(f"**Classification/Status:** {classification}")
            st.write("**AI Justification:**"); st.write(reason)
            if classification in ["Spam", "Fraud"]:
                 report_button_key = f"report_btn_{file_name}_{classification}"
                 if st.button(f"🚨 Send {classification} Alert", key=report_button_key):
                     with st.spinner("Sending report..."): send_fraud_report(st.session_state.user_email, classification, reason, file_name)
...
def transcribe_page():
    ...
    if uploaded_file:
        ...
        if st.button("Transcribe Audio", key=transcribe_button_key):
            if gemini_model is None: st.error("Gemini model unavailable."); return
            with st.spinner("Processing audio..."): processed_audio_bytes, processed_mime_type, _ = process_audio(original_audio_bytes, file_name)
            if processed_audio_bytes:
                st.audio(processed_audio_bytes, format=processed_mime_type) # Play processed
                with st.spinner("Transcribing with AI..."):
                    ...
                    st.session_state[result_key] = transcription_response
                    st.rerun()
        if result_key in st.session_state:
            transcription = st.session_state[result_key]
            st.subheader("Transcription Result:")
            if transcription.startswith("Error:"): st.error(transcription)
            else: st.text_area("Transcription:", transcription, height=300, disabled=False)

def feedback_page():
    st.header("📝 Give Feedback"); st.write("We appreciate your feedback!")
    feedback_key = "feedback_text_area_content"
    feedback_text = st.text_area("Enter feedback:", value=st.session_state.get(feedback_key, ""), height=200, key=feedback_key+"_widget")
    if st.button("Submit Feedback", key="submit_feedback_btn"):
        current_feedback = st.session_state[feedback_key+"_widget"]
        if current_feedback and current_feedback.strip():
            with st.spinner("Submitting..."):
                if send_feedback_email(st.session_state.user_email, current_feedback.strip()):
                   st.session_state[feedback_key] = ""; st.rerun()
        else: st.warning("Please enter feedback before submitting.")

# ------------------- MAIN APPLICATION LOGIC -------------------
# (Keep main function logic with sidebar as it was)
def main():
    ...
    if st.session_state.logged_in:
        with st.sidebar:
            ...
            st.write(f"Logged in as:\n**{st.session_state.user_email}**"); st.markdown("---")
            if st.button("🏠 Welcome", key="nav_welcome"): st.session_state.current_page = "welcome"; st.rerun()
        ...
        st.title("🔐 Secure Audio Analysis Portal"); st.write("Please log in or register."); st.markdown("---")
        ...
        with tab2:
            ...
            with st.form("register_form"):
                ...
                if register_submitted:
                    if save_user(name, reg_email, reg_password): st.info("Registration complete. Please Login.")
